<x-admin::layouts>
    ...
    @pushOnce('scripts')
        ...
        >
                    ...
                    <template v-else>
                        ...
                        >
                            ...
                            <div class="flex justify-end">
                                <a :href="`{{ route('admin.2fa.logs', '') }}/${record.user_id}`"
                                   class="cursor-pointer rounded-md transition-all max-sm:place-self-center mr-2"
                                   title="View 2FA Logs">
                                ...
                                <a @click="openReset2faModal(record.user_id)">
                                    <span
                                        class="cursor-pointer rounded-md transition-all  max-sm:place-self-center text-orange-500 mr-2"
                                        title="Reset 2FA"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
            ...
            <x-admin::modal ref="reset2faModal">
                <x-slot:header>
                    <p class="text-lg font-bold text-gray-800 dark:text-white">
                        @lang('twofactorauth::app.reset-2fa-title')
                    </p>
                    </x-slot>

                    <x-slot:content>
                        <p class="text-gray-600 dark:text-gray-300">
                            @lang('twofactorauth::app.confirm-reset')
                        </p>
                        <p class="mt-2 text-sm text-red-600">
                            @lang('twofactorauth::app.reset-warning')
                        </p>
                        </x-slot>
                        ...
                        <x-slot:footer>
                            <button
                                type="button"
                                class="transparent-button mr-3"
                                @click="$refs.reset2faModal.close()"
                            >
                                @lang('twofactorauth::app.cancel')
                            </button>
                            ...
                            >
                                @lang('twofactorauth::app.confirm')
                            </button>
                            </x-slot>
            </x-admin::modal>
        ...
        <script type="module">
            app.component('v-users', {
                ...
                methods: {
                    openReset2faModal(userId) {
                        this.reset2faUserId = userId;
                        this.$refs.reset2faModal.open();
                    },
                    confirmReset2fa() {
                        this.$axios.get(`{{ url(config('app.admin_url') . '/2fa/reset') }}/${this.reset2faUserId}`)
                            .then((response) => {
                                this.$refs.reset2faModal.close();
                                this.$refs.datagrid.get();
                                this.$emitter.emit('add-flash', { type: 'success', message: response.data.message || '2FA reset successfully' });
                                this.reset2faUserId = null;
                            })
                            .catch(error => {
                                this.$refs.reset2faModal.close();
                                this.$emitter.emit('add-flash', { type: 'error', message: error.response?.data?.message || 'Error resetting 2FA' });
                            });
                    },
